...
// What the number represents: (Followers count of the specified user) * (Count of retweets(within the specified count) of whichever profile is selected)
...
// $tweets = getTweets('twitter', 10);

// $ids = getRetweetersForEachTweet($tweets);


// Json Encode the array for testing later without having the headache of twitter api limit
...
foreach ($topUsers as $user) {
    echo "<img src=\"" . $user['profile_image_url'] . "\" /> <br/>";
}
...
function getRetweetersForEachTweet(&$tweets){
  ...
  foreach ($tweets as &$tweet) {
    ...
    // $all_ids = array_unique(array_merge($ids['ids'], $all_ids));
  }
  return $all_ids;
}
